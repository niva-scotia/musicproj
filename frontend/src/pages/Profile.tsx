...
import { Star, Music, Lock, Edit, Plus, Check, Loader2 } from 'lucide-react';
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        setPlaylists((playlistsRes.data.playlists || []).filter((p: Playlist) => isOwnProfile || p.isPublic));
      } catch {
  ...
  const fetchConnections = async (type: 'followers' | 'following') => {
    if (!profileId) return;
    try {
      const res = type === 'followers' ? await socialApi.getFollowers(profileId) : await socialApi.getFollowing(profileId);
      if (type === 'followers') {
        ...
        <div className="flex flex-col sm:flex-row gap-6">
          <div className="shrink-0">
            {profile.profilePictureUrl ? (
              <img src={profile.profilePictureUrl} alt={profile.username} className="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-gray-800" />
            ) : (
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex gap-2">
                {isOwnProfile ? (
                  <Link to="/settings" className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg transition-colors flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`px-6 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${
                      profile.isFollowing ? 'bg-gray-800 hover:bg-gray-700 text-white' : 'bg-indigo-600 hover:bg-indigo-700 text-white'
                    }`}
                  >
                    {isFollowLoading ? (
            ...
            <div className="flex gap-6 mt-4">
              {tabs.map((tab) => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)} className="text-center hover:opacity-80 transition-opacity">
        ...
        <div className="flex gap-1">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-3 font-medium transition-colors relative ${activeTab === tab.key ? 'text-white' : 'text-gray-400 hover:text-gray-200'}`}
            >
              {tab.label}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {playlists.length === 0 ? (
            ...
            playlists.map((playlist) => (
              <Link key={playlist.id} to={`/playlist/${playlist.id}`} className="bg-gray-900 rounded-xl p-4 border border-gray-800 hover:border-gray-700 transition-colors">
      ...
      {(activeTab === 'followers' || activeTab === 'following') && (
        <div className="space-y-3">
          {(activeTab === 'followers' ? followers : following).length === 0 ? (
            <div className="text-center py-12 bg-gray-900 rounded-xl border border-gray-800">
              <p className="text-gray-400">{activeTab === 'followers' ? 'No followers yet' : 'Not following anyone yet'}</p>
            </div>
          ) : (
            (activeTab === 'followers' ? followers : following).map((user) => (
              <Link key={user.id} to={`/profile/${user.id}`} className="flex items-center gap-4 p-4 bg-gray-900 rounded-xl border border-gray-800 hover:border-gray-700 transition-colors">
